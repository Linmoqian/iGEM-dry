...
def inner_group_folds(
    metadata: pd.DataFrame, n_folds: int, seed: int, outer_protocol: str = "waterbody_ood"
) -> np.ndarray:
    """Mirror source shift inside source-OOD runs; otherwise hold out waterbodies."""
    if outer_protocol == "source_ood":
        groups = metadata["source_group"]
        available = int(groups.nunique())
        if available >= 2:
            return balanced_group_folds(groups, n_folds=min(n_folds, available), seed=seed)
