...
风应力公式在代码里是 `swe_si.py` 的 `wind_stress()`：气象学惯例"风从哪来"（0°=北风），`τ = 1.225 × Cd × W²`，分量取 `(-sinθ, -cosθ)`。**Cd 默认用湖面式 `cd_lake(U)`**（Zhang 2024 Fig.4b 拟合：2.5 m/s → 2.46e-3；常数 1.3e-3 已弃用，见 E9）；`cd_mode='wu'`（Wu 1980）与 'constant' 保留作对照。
...
| 量级对照 | 浅湖风生流 0.03–0.07 m/s（B3+湖面Cd 生产口径） | SE 2.5 m/s 风 → max **0.068 m/s** ✅ |
...
历史：最初的纯指数剖面 d = Dmax·(1−e^(−dist/L))（L≈381–498 m 随标定目标变）只有 1 个参数，形态是"中心尖坑"。**2026-08-24 起生产采用实测标定的幂指数剖面（B3）**：

```
d(x) = 4.66 · (1 − e^(−(dist(x)/440)^0.39))
```

- 数据来源：**刘惠 et al. (2019) 的 42 个东湖实测水深点**（本模块已数字化，脚本 12/13 内嵌）；
- 两个标定锚：**湖均值 2.48 m、最大 4.66 m**（刘惠：33.2 km² / 1.24 亿 m³）；
- L=440、p=0.39 由 42 点 + 均值约束最小二乘求得；**42 点 RMSE 1.02 → 0.49 m（−52%），LOOCV 0.478**（E0/E8）；旧指数/线性（GLOBathy 式）与无约束自由拟合被明确否定（E0-b）；
- **为什么选幂指数**：近岸陡升 + 平台化湖盆，与"郭郑湖/团湖/后湖中心最深、小湖汊<2 m"的实测描述一致；参数少、单调光滑。它仍**不是**实测等深（42 点外的细节是插值），环流"模式/方向"可信、"幅值"以流速量级对照为上限（E7）——这是模型文档反复强调的第一局限。

**口径注意**：① 湖内实际最大 **3.70 m**（4.66 为渐近标定目标，湖内不达）；② 测量水深含淤（平均淤泥厚 1.06 m），对"硬底地形"有 0.5–1 m 级差异，计入不确定度。
...
- 场景设计：选三个基态（SE 2.5 m/s 夏季东南风基线、N 3.0 m/s 冬春北风、W 2.0 m/s 西风），每个场景用半隐式求解器自旋 24–36 h 到准稳态；2026-08-24 起新增分位成员 `flow_SE_p75`(3.33 m/s,131°) 与 `flow_N_p75`(5.17 m/s,4°)（E12；注意 cd_lake 于 3.4 m/s 封顶 3.6e-3，高风成员强度由 clip 决定）。
...
- 5% 仍是默认演示值；**物理阈值由 `deployment.physical_thr_rel(C0, t_ok, T_water, M)` 反算**（降解模块 m6 动力学：东湖典型 C0=2.85 µg/L、24 h、25°C、M=1 → thr_rel=**0.2465**，是 5% 的 4.9 倍；在该阈值下单架次覆盖坍缩 3.2 倍、最优投放点移向上风侧 1.0 km——E11）。**引用面积数字必须标注 thr 口径**（demo/physical）。
...
```
dx=50 m | dt=20 s（流场）/ 5–10 s（粒子）| n=0.0238 | Cd=cd_lake(U)≈1.8–3.1e-3 | ν(Smag 0.29)
f=7.42e-5 s⁻¹ | D=0.3 m²/s | σ0=25 m | windage=2% | thr_rel=5%演示/0.2465物理 | 自旋 24–36 h | KDE σ=30 m
```
...
6. fig01 标题错字已修（domain.py 现为"武汉东湖"，fig01 已含 B3 三面板）；fig05 的 Manning 占位面板**仍未补绘**（控制台有数字，图件缺）——唯一未清的旧 P2 项。
...
| C_req | 降解动力学模块 → 本模块 | 标量，`physical_thr_rel()` 反算 thr_rel（E11 联标演示完成） | ✅ **已实现**（`deployment.physical_thr_rel`；04 支持 --physical） |
