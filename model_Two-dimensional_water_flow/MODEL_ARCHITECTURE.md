...
- **采用方案**：掩膜内"离岸距离"递变换 + 指数剖面 \(d = D_{max}(1-e^{-dist/L})\)；**标定目标改为实测值（东湖 MIKE21 论文/湖区志：平均 2.21 m、最大 4.75 m）**，自动求 \(L≈381\) m；场上实际最大值 4.64 m（指数剖面在湖内达不到渐近目标，属标定特性，非误差）。特征：边浅中深、北浅南深（与文献"北浅南深"一致）。
